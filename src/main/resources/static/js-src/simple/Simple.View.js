    ...
    function(Simple) {
        Simple.View = Arm.create('View', {
            ...
            init: function() {
                ...
                // 也可以在实例化后在设置ArrayList，见class中rowsData声明处
                // view中的rowsData与class中的employeeList基本相同，所不同的是
                // employeeList是面向具体业务properties的ArrayList,
                // rowsData来自view的options，面向DOM渲染
                this.class.setEmployeeList(this.options.rowsData);
                this.class.setOrignEmployeeList(this.options.rowsData);
            ...
            bindFetch: function() {
                ...
                self.class.getEmployeeList().onFetch = function(data) {
                    self.logger.log('bindFetch', 'Arguments:', arguments);
                    ...
                    // 组合测试，list2来自class调用自OtherClass的方法，获取的是原始数据
                    var list2 = self.class.getOrignEmployeeList().orderBy('name', 'desc');
                    // 分别打印Class的List与OtherClass的list, OtherClass里是原始值
                    self.logger.log('bindFetch', '第一次对比:', list, list1, list2);

                    // 现在给getEmployeeList函数添加bind，更改this为class，再对比下数据
                    var getList = self.action.get('OtherClass').getEmployeeList.bind(self.class);
                    self.logger.log('bindFetch', '第二次对比:', list.orderBy('id', 'desc').get(0), getList().orderBy('id', 'desc').get(0), list1.get(0), list2.get(0));
                    // 可以看到list与getList()数据一致了，list1与list2排序不同，但数据还是原来的
                    // 这就是通过bind来改变函数的调用，以及通过组合来调用方法的例子
            ...
            bindEvent: function() {
                var self = this;
                // 本事件不是代理，而是页面必然存在的按钮且不会被移动，比如提交按钮之类
            ...
            checkSexRadio: function(evt, ele) {
                ...
                this.logger.log('checkSexRadio', '点击了"' + sexText + '"按钮');
                // id,value的判断都通过方法来实现
                if (this.action.getUtil().isFemale(value)) {
                    this.logger.log('checkSexRadio', '你确实点的是female。');
                }
            },
            ...
            addItem: function(evt, ele) {
                ...
                // 通过专门的表单处理控件Formit来获取字段，比如过滤项，取data-id, 现在统一使用QUI中formit插件
                var data = this.$form.formit('getParamObj');
                id = data.id;
            ...
            onTableUpdated: function() {
                this.logger.log('onTableUpdated', 'This:', this, 'Message:', '更新完成，可以执行有关操作了', 'Arguments:', arguments);
            },
            ...
            updateContent: function(evt, ele) {
                ...
                var callback = function(data) {
                    self.logger.log('updateContent.callback', 'Data:', data, 'EmployeeList:', self.class.getEmployeeList());
            ...
            sortTableData: function(evt, ele) {
                ...
                self.logger.log('sortTableData', 'Field:', field, 'Order:', order);
                // evt.preventDefault();
                // evt.stopPropagation();
                return false;
            },
